...
<section class="panel">
    <div class="panel-header no-print">
        ...
        <div class="actions">
            ...
            <?php endif; ?>
            <?php if (\App\Core\Permission::can('work_plans.approve') && $plan['status'] !== 'approved'): ?>
                <form method="post" action="/work-plans/<?= e((string) $plan['id']) ?>/approve">
                    <?= csrf_field() ?>
    ...
    <table class="meta-table">
        ...
        <tr>
            <th>計畫目標</th>
            <td colspan="3"><?= nl2br(e($plan['objective'] ?: '-')) ?></td>
        </tr>
        <tr>
            <th>服務對象</th>
            <td colspan="3"><?= nl2br(e($plan['target_audience'] ?: '-')) ?></td>
        </tr>
        <tr>
            <th>服務區域</th>
            <td colspan="3"><?= e($plan['service_area'] ?: '-') ?></td>
        </tr>
        <tr>
            <th>預期成果</th>
            <td colspan="3"><?= nl2br(e($plan['expected_outcomes'] ?: '-')) ?></td>
        </tr>
        <tr>
            <th>成果指標</th>
            <td colspan="3"><?= nl2br(e($plan['performance_indicators'] ?: '-')) ?></td>
        </tr>
        </tbody>
    </table>
    ...
    <div class="table-wrap">
        <table>
            ...
            <?php endforeach; ?>
            <?php if (!$items): ?>
                <tr><td colspan="5" class="empty">尚無工作項目。</td></tr>
            <?php endif; ?>
            </tbody>
            <tfoot>
            <tr>
                <th colspan="4">預算合計</th>
                <th class="amount"><?= e(number_format((float) $budgetTotal, 0)) ?></th>
            </tr>
            </tfoot>
        </table>
    </div>
