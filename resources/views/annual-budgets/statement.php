    ...
    <div class="table-wrap">
        <table class="data-table statement-table">
            ...
            <tr>
                ...
                <th rowspan="2">本年度<br>預算數(A)</th>
                <th rowspan="2">上年度<br>預算數(B)</th>
                <th colspan="2">本年度與上年度預算比較</th>
                <th rowspan="2">備註</th>
            </tr>
            <tr>
                <th>增(減)數<br>(C)=(A)-(B)</th>
                <th>增(減)比率%<br>(D)=(C)/(A)*100</th>
            </tr>
            </thead>
            <tbody>
            ...
            <?php if (!$items): ?>
                <tr><td colspan="11" class="empty-state">尚無預算明細。</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
...
function render_statement_section(string $type, string $label, array $statement): void
{
    ...
    foreach ($statement['groups'][$type] as $group) {
        ...
        foreach ($group['items'] as $item) {
            ...
            echo '<td class="amount">' . e(statement_money($item['amount'])) . '</td>';
            echo '<td class="amount">' . e(statement_money($item['previous_amount'] ?? 0)) . '</td>';
...
function render_statement_total(string $label, array $row): void
{
    ...
    echo '<td class="amount">' . e(statement_money($row['current'])) . '</td>';
    echo '<td class="amount">' . e(statement_money($row['previous'])) . '</td>';
